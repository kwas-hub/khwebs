...
const ApiSettingsPage = () => {
  ...
  const load = async () => {
    ...
    // 🔑 Nur Endpunkte des aktuellen Mandanten laden
  ...
  useEffect(() => { 
    load(); 
  }, [currentTenant?.id]); // 🔑 Bei Mandanten-Wechsel neu laden
  ...
  const save = async () => {
    ...
    let error;
    if (editing.id) {
      // 🔑 Sicherheitshalber auch tenant_id prüfen
  ...
  const testExport = async (ep: Endpoint) => {
    ...
    const { data, error } = await supabase.functions.invoke("export-record", {
      body: { 
        ...
        test: true,
        tenant_id: currentTenant.id // 🔑 Mandant übergeben
      },
    });
    toast.dismiss("test");
    if (error) return toast.error(error.message);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          {isTenantAdmin && (
            <Button onClick={() => setEditing(empty(currentTenant.id))}>
              <Plus className="w-4 h-4 mr-1" />Neuer Endpunkt
            </Button>
          )}
        </div>
                      ...
                      {items.map(ep => (
                        <div key={ep.id} className="flex items-center justify-between p-3 border rounded-lg gap-3">
                          ...
                          <div className="flex gap-1">
                            <Button size="sm" variant="ghost" onClick={() => testExport(ep)}>
                  ...
                  <TableBody>
                    {queue.map(q => (
                      <TableRow key={q.id}>
                        ...
                        <TableCell>
                          <Badge 
                            variant={q.status === "success" ? "default" : q.status === "failed" ? "destructive" : "secondary"} 
                            className="text-[10px]"
                          >
                            {q.status}
                          </Badge>
