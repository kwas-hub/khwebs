...
const Formulare = () => {
  ...
  const moveField = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      const { error: err1 } = await supabase.from("form_fields").update({ position: currentField.position }).eq("id", currentField.id);
      const { error: err2 } = await supabase.from("form_fields").update({ position: swapField.position }).eq("id", swapField.id);
  ...
  const updateSubmissionData = async (submissionId: string, key: string, newValue: string) => {
    ...
    await supabase.from("form_submissions").update({ data: updatedData }).eq("id", submissionId);
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button onClick={addForm} className="rounded-full shadow-lg"><Plus className="h-4 w-4 mr-2" />Neues Formular</Button>
        </div>
        ...
        <Tabs defaultValue="builder" className="w-full">
          ...
          <TabsContent value="builder" className="space-y-6 mt-6">
            <div className="grid md:grid-cols-[280px_1fr] gap-6">
              ...
              {activeForm && (
                <div className="space-y-6">
                  <Card className="p-6 space-y-4 border-border/60 shadow-sm relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t">
                      ...
                      <Button variant="destructive" size="sm" onClick={() => deleteForm(activeForm.id)} className="rounded-lg"><Trash2 className="h-4 w-4 mr-2" />Löschen</Button>
                    </div>
                  </Card>

                  <Card className="p-6 space-y-6 border-border/60 shadow-sm">
                    <div className="flex items-center justify-between border-b pb-4">
                      <h3 className="font-bold text-lg tracking-tight">Felder</h3>
                      <div className="flex flex-wrap gap-1.5">
                        {(["text","number","email","textarea","radio","checkbox","html"] as FieldType[]).map((t) => (
                          <Button key={t} size="sm" variant="secondary" onClick={() => addField(t)} className="h-8 text-[10px] font-bold uppercase tracking-wider rounded-md">+ {t}</Button>
                        ))}
                      </div>
                    </div>
                        ...
                        <Card key={f.id} className="p-4 space-y-4 bg-muted/30 border-border/40 group hover:border-primary/30 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-zinc-200 dark:bg-zinc-800 uppercase tracking-widest">{f.field_type}</span>
                              <span className="text-[10px] font-mono text-muted-foreground bg-background px-2 py-0.5 rounded border italic">ID: {f.field_name}</span>
                            </div>
                            <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => deleteField(f.id)}><Trash2 className="h-4 w-4" /></Button>
                          </div>

                          {f.field_type === "html" ? (
                            <div className="space-y-2">
                              <Label className="text-[10px] font-bold uppercase text-muted-foreground">HTML / Javascript</Label>
                              <Textarea className="font-mono text-xs bg-zinc-950 text-green-400 rounded-lg p-4" rows={8} value={f.html_content} onChange={(e) => updateField(f.id, { html_content: e.target.value })} />
                            </div>
                          ) : (
                            <>
                              <div className="grid sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase text-muted-foreground">Label</Label><Input className="h-9" value={f.label} onChange={(e) => updateField(f.id, { label: e.target.value })} /></div>
                                <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase text-muted-foreground">Platzhalter</Label><Input className="h-9" value={f.placeholder} onChange={(e) => updateField(f.id, { placeholder: e.target.value })} /></div>
                              </div>
                              {(f.field_type === "radio" || f.field_type === "checkbox") && (
                                <div className="space-y-3 pt-4 border-t border-border/40">
                                  <Label className="text-[10px] font-bold uppercase text-muted-foreground">Optionen</Label>
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {f.options.map((opt, idx) => (
                                      <div key={idx} className="flex gap-2">
                                        <Input className="h-8 bg-background text-sm" value={opt} onChange={(e) => updateOption(f.id, idx, e.target.value)} />
                                        <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => removeOption(f.id, idx)}><Trash2 className="h-3.5 w-3.5" /></Button>
                                      </div>
                                    ))}
                                    <Button size="sm" variant="outline" className="h-8 text-[10px] font-bold uppercase border-dashed" onClick={() => addOption(f.id)}>+ Option</Button>
                                  </div>
                                </div>
                              )}
                            </>
          ...
          <TabsContent value="submissions" className="mt-6">
            <Card className="overflow-hidden bg-card border-border/60 shadow-sm">
              <Table>
                <TableHeader className="bg-muted/50 text-[11px] uppercase font-bold tracking-wider">
                  <TableRow><TableHead>Datum</TableHead><TableHead>Formular</TableHead><TableHead>Inhalt</TableHead><TableHead className="w-10"></TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {subs.map((s) => (
                    <TableRow key={s.id} className="align-top">
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap pt-4">{new Date(s.created_at).toLocaleString("de-DE")}</TableCell>
                      <TableCell className="font-bold text-sm pt-4">{forms.find(f => f.id === s.form_id)?.title || "—"}</TableCell>
                      <TableCell className="py-2">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                          {Object.entries(s.data).map(([k, v]) => (
                            <div key={k} className="p-2 bg-muted/30 rounded-lg border border-border/40">
                              <p className="text-[9px] font-bold uppercase text-muted-foreground mb-1">{k}</p>
                              <Input className="h-7 text-xs bg-background" value={String(v)} onChange={(e) => updateSubmissionData(s.id, k, e.target.value)} />
                            </div>
                          ))}
                        </div>
                      </TableCell>
                      <TableCell className="pt-2"><Button size="icon" variant="ghost" className="text-destructive h-8 w-8" onClick={() => deleteSubmission(s.id)}><Trash2 className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                  {subs.length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-20 text-muted-foreground italic">Keine Einsendungen vorhanden.</TableCell></TableRow>}
                </TableBody>
              </Table>
            </Card>
          </TabsContent>
